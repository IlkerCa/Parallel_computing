...
void commonSingleBorder(std::vector<std::vector<int>> &dwellBuffer, std::complex<double> const &cmin,
	std::complex<double> const &dc, int atY, int atX, int blockSize, int s, int &dwell) {
	...
	for (unsigned int i = 0; i < blockSize; i++) {
		...
		if (y < res && x < res) {
			...
			if (dwell == -1) {
				dwell = dwellBuffer.at(y).at(x);
			}
			else if (dwell != dwellBuffer.at(y).at(x)) {
				dwell = -1;
				return ;
			}
		}
...
// task 1b:
//Use std::thread and std::atomic to implement a multi
//threaded common border evaluation function, in which each thread is responsible for
//one border.
int threadCommonBorder(std::vector<std::vector<int>> &dwellBuffer,
	...
	unsigned int const blockSize)
{
	...
	//first initialise and assign the value!
	std::atomic<int> commonDwell;
	commonDwell = -1;

	std::thread threadForBorders[4];
	int dwells[4];

		for (unsigned int s = 0; s < 4; s++) {

			threadForBorders[s] = std::thread(commonSingleBorder,
				std::ref(dwellBuffer),
				std::ref(cmin),
				std::ref(dc),
				atY,
	...
	for (int i = 0; i < 4; i++) {
		threadForBorders[i].join();
	}

	for(int i = 0; i < 3; i++){
		if(dwells[i +1] != dwells[i +1]){
			return -1;
		}
	}
	commonDwell = dwells[0];
...
void computeBlockLoop(std::vector<std::vector<int>> &dwellBuffer,
	std::complex<double> const &cmin,
	std::complex<double> const &dc,
	unsigned int xend,
	unsigned int ystart,
	...
	unsigned int const omitBorder = 0)
{
	...
	std::thread threadForComputeBlock[num_thread];

	// dividing resolution in horizontal rectangles
	unsigned int splittedY = yMax / num_thread;
	unsigned int currentOfY = 0;
	unsigned int endOfY = 0;


	//updating rectangles borders
	//calculating the lopp in another function
	for (int i = 0; i < num_thread; i++) {
		endOfY = splittedY + endOfY;

		if(i < num_thread - 1 ){
				threadForComputeBlock[i] = std::thread(computeBlockLoop,
					std::ref(dwellBuffer),
					cmin,
					dc,
					xMax,
...
{
	//task 1b
	//int dwell = threadCommonBorder(dwellBuffer, cmin, dc, atY, atX, blockSize);
