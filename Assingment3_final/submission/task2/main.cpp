	...
	if ( dwell >= 0 ) {
		...
		for (unsigned int ydiv = 0; ydiv < subDiv; ydiv++) {
			for (unsigned int xdiv = 0; xdiv < subDiv; xdiv++) {
				job j{dwellBuffer, cmin, dc, atY + (ydiv * newBlockSize), atX + (xdiv * newBlockSize), newBlockSize};
				addWork(j);
...
void worker(void) {
	while (1) {
		// this tries to lock the mutex actively in a loop
		std::unique_lock<std::mutex> lock(mut);
		// forces the thread to "sleep" and waits until it gets notified
		// if the condition is true, waiting is finished
		// else it checks the condition actively in a loop
		sleeper.wait(lock, [] { return (!queue.empty() || counter == 0 && queue.empty());});
