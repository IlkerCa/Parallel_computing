...
__device__ void rasteriseTriangle( float4 &v0, float4 &v1, float4 &v2,
	...
	minx = min(max(minx, (unsigned int) 0), width);
	maxx = max(min(maxx, width), 0);
	miny = min(max(miny, (unsigned int) 0), height);
	maxy = max(min(maxy, height), 0);

	// CUSTOM CODE //
	unsigned int const THRESHOLD = 64;
	unsigned int amount_of_pixels_of_bounding_box = (maxx - minx) * (maxy - miny);
	int big_triangles = __ballot_sync(0xFFFFFFFF, amount_of_pixels_of_bounding_box >= THRESHOLD);

	for (unsigned int j = 0; j < __popc(big_triangles); j++) {
		int pos = __ffs(big_triangles) - 1;
		// exchange triangle index
		unsigned int triangleIdx = __shfl_sync(0xFFFFFFFF, triangleIndex, pos);
		// broadcast thread-dependent parameters
		int minxlol = __shfl_sync(0xFFFFFFFF, minx, pos);
		int maxxlol = __shfl_sync(0xFFFFFFFF, maxx, pos);
		int minylol = __shfl_sync(0xFFFFFFFF, miny, pos);
		int maxylol = __shfl_sync(0xFFFFFFFF, maxy, pos);
		float4 v0lol, v1lol, v2lol;
		v0lol.x = __shfl_sync(0xFFFFFFFF, v0.x, pos);
		v0lol.y = __shfl_sync(0xFFFFFFFF, v0.y, pos);
		v0lol.z = __shfl_sync(0xFFFFFFFF, v0.z, pos);
		v1lol.x = __shfl_sync(0xFFFFFFFF, v1.x, pos);
		v1lol.y = __shfl_sync(0xFFFFFFFF, v1.y, pos);
		v1lol.z = __shfl_sync(0xFFFFFFFF, v1.z, pos);
		v2lol.x = __shfl_sync(0xFFFFFFFF, v2.x, pos);
		v2lol.y = __shfl_sync(0xFFFFFFFF, v2.y, pos);
		v2lol.z = __shfl_sync(0xFFFFFFFF, v2.z, pos);
		unsigned int boundingWidth = maxxlol - minxlol;
		unsigned int amount_of_pixels_of_bounding_box = boundingWidth * (maxylol - minylol);

		for (unsigned int i = 0; i < amount_of_pixels_of_bounding_box; i += 32) {
			unsigned int x = minxlol + (i + threadIdx.x) % boundingWidth;
			unsigned int y = minylol + (i + threadIdx.x) / boundingWidth;
			rasteriseSinglePixel(v0lol, v1lol, v2lol, x, y, mesh, triangleIdx, frameBuffer, depthBuffer, width);
		}

		// set bit at pos to 0
		big_triangles = big_triangles ^ (1U << pos);
	}

	// h)
	if (amount_of_pixels_of_bounding_box >= THRESHOLD) {
		return;
	}
	...
	for (unsigned int x = minx; x < maxx; x++) {
		for (unsigned int y = miny; y < maxy; y++) {
				rasteriseSinglePixel(v0, v1, v2, x, y, mesh, triangleIndex, frameBuffer, depthBuffer, width);
		}
	}
}
