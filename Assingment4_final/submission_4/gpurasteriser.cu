...
void renderMeshesKernel(
		...
		unsigned int length = totalItemsToRender * meshCount;
		if (i < length) {
			unsigned int item = i / meshCount;
			unsigned int meshIndex = i % meshCount;
...
__global__
void initDbKernel(int* buffer, unsigned int length) {
	unsigned int i = threadIdx.x + blockIdx.x * blockDim.x;
	if (i < length) {
		buffer[i] = 16777216;
	}
...
__global__
void initFbKernel(unsigned char* buffer, unsigned int length) {
	unsigned int i = 4 * (threadIdx.x + blockIdx.x * blockDim.x);
	if (i < length) {
		buffer[i] = 0;
		buffer[i+1] = 0;
		buffer[i+2] = 0;
...
std::vector<unsigned char> rasteriseGPU(std::string inputFile, unsigned int width, unsigned int height, unsigned int depthLimit) {
    ...
    checkCudaErrors(cudaGetDeviceCount(&deviceCount));
  	...
  	checkCudaErrors(cudaSetDevice(0));
		...
		std::size_t fb_size_in_bytes = sizeof(unsigned char) * width * height * 4;
		checkCudaErrors(cudaMalloc(&gpu_fb, fb_size_in_bytes));
		checkCudaErrors(cudaDeviceSynchronize());
		// depthbuffer
		int* gpu_db;
		std::size_t db_size_in_bytes = sizeof(int) * width * height;
		checkCudaErrors(cudaMalloc(&gpu_db, db_size_in_bytes));
		checkCudaErrors(cudaDeviceSynchronize());

		//task 4b
		// depthbuffer
		unsigned int grid = (width * height / 1024) + 1;
		initDbKernel<<<grid, 1024>>>(gpu_db, width*height);
		checkCudaErrors(cudaDeviceSynchronize());

		// print
		/*
		int* test_db = new int[width * height];
    cudaMemcpy(test_db, gpu_db, db_size_in_bytes, cudaMemcpyDeviceToHost);
    for (std::size_t i =0; i < 100; i++) {
      printf("%d - ", test_db[i]);
    }
		printf("----------------------------\n");
		for (std::size_t i =width*height - 100; i < width*height; i++) {
      printf("%d - ", test_db[i]);
    }
		checkCudaErrors(cudaDeviceSynchronize());
		free(test_db);
		*/
		...
		// framebuffer
		grid = (width * height / 1024) + 1;
		initFbKernel<<<grid, 1024>>>(gpu_fb, 4*width*height);
		checkCudaErrors(cudaDeviceSynchronize());
		// print
		/*
		unsigned char* test_fb = new unsigned char[4 * width * height];
    cudaMemcpy(test_fb, gpu_fb, fb_size_in_bytes, cudaMemcpyDeviceToHost);
    for (std::size_t i =0; i < 100; i++) {
      printf("%d - ", test_fb[i]);
    }
		printf("----------------------------\n");
		for (std::size_t i =4*width*height - 100; i < 4*width*height; i++) {
      printf("%d - ", test_fb[i]);
    }
		checkCudaErrors(cudaDeviceSynchronize());
		free(test_fb);
		*/
		...
		std::size_t workQueue_size_in_bytes = sizeof(workItemGPU) * totalItemsToRender;
		checkCudaErrors(cudaMalloc(&gpu_workQueue, workQueue_size_in_bytes));
		checkCudaErrors(cudaDeviceSynchronize());
		checkCudaErrors(cudaMemcpy(gpu_workQueue, workQueue, workQueue_size_in_bytes, cudaMemcpyHostToDevice));
		checkCudaErrors(cudaDeviceSynchronize());

		// print
		/*
		workItemGPU* test_workQueue = new workItemGPU[totalItemsToRender];
    cudaMemcpy(test_workQueue, gpu_workQueue, workQueue_size_in_bytes, cudaMemcpyDeviceToHost);
    for (std::size_t i =0; i < 100; i++) {
      printf("%f - ", test_workQueue[i].scale);
			printf("%f %f %f - ", test_workQueue[i].distanceOffset.x, test_workQueue[i].distanceOffset.y, test_workQueue[i].distanceOffset.z);
    }
		printf("----------------------------\n");
		for (std::size_t i =totalItemsToRender - 50; i < totalItemsToRender; i++) {
			printf("%f - ", test_workQueue[i].scale);
			printf("%f %f %f - ", test_workQueue[i].distanceOffset.x, test_workQueue[i].distanceOffset.y, test_workQueue[i].distanceOffset.z);
    }
		checkCudaErrors(cudaDeviceSynchronize());
		...
		*/
		...
		//task 4d
		GPUMesh* cpu_meshes = new GPUMesh[meshes.size()];
		GPUMesh* gpu_meshes;
		std::size_t meshes_size_in_bytes = sizeof(GPUMesh) * meshes.size();
		checkCudaErrors(cudaMalloc(&gpu_meshes, meshes_size_in_bytes));
		checkCudaErrors(cudaDeviceSynchronize());

		for (std::size_t i = 0; i < meshes.size(); i++) {
				// allocate vertices and normals on GPU
				float4* gpu_vertices;
				checkCudaErrors(cudaMalloc(&gpu_vertices, sizeof(float4) * meshes.at(i).vertexCount));
				checkCudaErrors(cudaDeviceSynchronize());
				float3* gpu_normals;
				checkCudaErrors(cudaMalloc(&gpu_normals, sizeof(float3) * meshes.at(i).vertexCount));
				checkCudaErrors(cudaDeviceSynchronize());
				...
				cpu_meshes[i].vertexCount = meshes.at(i).vertexCount;
				cpu_meshes[i].objectDiffuseColour = meshes.at(i).objectDiffuseColour;
				cpu_meshes[i].hasNormals = meshes.at(i).hasNormals;
		}

		checkCudaErrors(cudaMemcpy(gpu_meshes, cpu_meshes, sizeof(GPUMesh) * meshes.size(), cudaMemcpyHostToDevice));
		checkCudaErrors(cudaDeviceSynchronize());

		//task 5a
		grid = (totalItemsToRender * meshes.size() / 1024) + 1;
		auto start = std::chrono::high_resolution_clock::now();
		renderMeshesKernel<<<grid, 1024>>>(
			totalItemsToRender,
			gpu_workQueue,
			gpu_meshes,
			meshes.size(),
		...
		checkCudaErrors(cudaDeviceSynchronize());
		checkCudaErrors(cudaMemcpy(frameBuffer, gpu_fb, fb_size_in_bytes, cudaMemcpyDeviceToHost));
		checkCudaErrors(cudaDeviceSynchronize());

		std::cout << "Time taken: " << time << " microseconds" << std::endl;

		checkCudaErrors(cudaFree(gpu_fb));
		checkCudaErrors(cudaFree(gpu_db));
		checkCudaErrors(cudaFree(gpu_meshes));
		checkCudaErrors(cudaFree(gpu_workQueue));
		checkCudaErrors(cudaDeviceSynchronize());
		/************* /GPU CODE *************/
